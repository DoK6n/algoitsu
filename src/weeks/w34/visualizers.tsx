    ...
    <div className="bg-[#080b07] border border-[#2a3325] rounded-[12px] p-5 my-[14px]">
      <div className="flex gap-8 items-end flex-wrap">
        ...
        >
          ...
          {stack.length === 0 ? (
            <div className="text-[#5a6e50] p-3 border border-dashed border-[#2a3325] rounded-[6px]">
              비어있음
            </div>
          ) : (
            [...stack].reverse().map((v, i) => (
              <div
                key={`stack-item-${v}-${i}`}
                className="w-[100px] h-[38px] flex items-center justify-center rounded-[6px] transition-all duration-300 font-mono text-[16px] text-[#dde8d6] border-2"
                style={{
                  background:
                    i === 0 && flash === "push"
                      ? "#7eff6a44"
        ...
        <div className="flex-1" style={{ minWidth: 200 }}>
          <div className="flex gap-2 mb-3 flex-wrap">
            ...
            <button
              type="button"
              onClick={push}
              className="px-4 py-2 bg-[#7eff6a] border-none text-black rounded-[8px] cursor-pointer font-bold"
            >
              push
            </button>
            <button
              type="button"
              onClick={pop}
              className="px-4 py-2 bg-[#ff4f4f] border-none text-white rounded-[8px] cursor-pointer font-bold"
            >
              pop
            </button>
            <button
              type="button"
              onClick={peek}
              className="px-4 py-2 bg-[#1a1f16] border border-[#2a3325] text-[#dde8d6] rounded-[8px] cursor-pointer"
            >
              peek
            </button>
          </div>
          {msg && (
            <div className="text-[#7eff6a] bg-[#7eff6a15] border border-[#7eff6a33] px-[14px] py-2 rounded-[8px] text-[13px] font-mono">
              {msg}
            </div>
          )}
  ...
  return (
    <div className="bg-[#080b07] border border-[#2a3325] rounded-[12px] p-5 my-[14px]">
      ...
      <div className="flex gap-1 items-center mb-4 overflow-x-auto pb-1">
        ...
        {queue.length === 0 ? (
          <div className="text-[#5a6e50] px-6 py-3 border border-dashed border-[#2a3325] rounded-[8px]">
            비어있음
          </div>
        ) : (
          queue.map((v, i) => (
            <div
              key={`queue-item-${v}-${i}`}
              className="flex items-center justify-center rounded-[8px] text-[#dde8d6] font-bold font-mono text-[16px] border-2"
              style={{
                minWidth: 52,
                height: 52,
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <button
          type="button"
          onClick={enqueue}
          className="px-4 py-2 bg-[#ff8c42] border-none text-black rounded-[8px] cursor-pointer font-bold"
        >
          enqueue
        </button>
        <button
          type="button"
          onClick={dequeue}
          className="px-4 py-2 bg-[#42c9ff] border-none text-black rounded-[8px] cursor-pointer font-bold"
        >
          dequeue
        </button>
        {msg && (
          <span className="text-[#7eff6a] text-[13px] py-2 font-mono">
            {msg}
          </span>
        )}
      </div>
    ...
    <div className="bg-[#080b07] border border-[#2a3325] rounded-[12px] p-5 my-[14px]">
      <div className="flex gap-1 items-center mb-[14px] overflow-x-auto">
        ...
        {deque.map((v, i) => (
          <div
            key={`deque-item-${v}-${i}`}
            className="flex items-center justify-center rounded-[8px] text-[#dde8d6] font-bold font-mono text-[16px] border-2 bg-[#1a1f16]"
            style={{
              minWidth: 52,
              height: 52,
              borderColor:
      ...
      <div className="flex gap-2 flex-wrap items-center">
        ...
        <button
          type="button"
          onClick={appendleft}
          className="px-3 py-[6px] bg-[#ff4f8bcc] border-none text-white rounded-[8px] cursor-pointer text-[12px] font-bold"
        >
          appendleft
        </button>
        <button
          type="button"
          onClick={popleft}
          className="px-3 py-[6px] bg-[#ff4f8b44] border border-[#ff4f8b] text-[#ff4f8b] rounded-[8px] cursor-pointer text-[12px]"
        >
          popleft
        </button>
        <button
          type="button"
          onClick={append}
          className="px-3 py-[6px] bg-[#b57fffcc] border-none text-white rounded-[8px] cursor-pointer text-[12px] font-bold"
        >
          append
        </button>
        <button
          type="button"
          onClick={pop}
          className="px-3 py-[6px] bg-[#b57fff44] border border-[#b57fff] text-[#b57fff] rounded-[8px] cursor-pointer text-[12px]"
        >
          pop
        </button>
      </div>
      {msg && (
        <div className="mt-[10px] text-[#7eff6a] font-mono text-[13px]">
          {msg}
        </div>
      )}
    </div>
...
function W34_HeapViz() {
    ...
    return (
      <g key={`heap-node-${i}`}>
        <circle
          ...
          cy={y}
          r={18}
          fill={i === 0 ? `${W34_T.accent}33` : W34_T.card}
          stroke={i === 0 ? W34_T.accent : W34_T.border}
          strokeWidth={2}
        />
        <text
          x={x}
  ...
  const svgHeight =
    heap.length > 0 ? Math.floor(Math.log2(heap.length)) * 54 + 60 : 60;
  ...
  return (
    <div className="bg-[#080b07] border border-[#2a3325] rounded-[12px] p-5 my-[14px]">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        ...
        {msg && (
          <span className="text-[#7eff6a] text-[13px] font-mono">{msg}</span>
        )}
      </div>
    </div>
  );
...
function W34_HashMapViz() {
  ...
  const bucketSize = 5;
  const hash = k =>
    k.split("").reduce((a, c) => a + c.charCodeAt(0), 0) % bucketSize;
  const buckets = Array.from({ length: bucketSize }, (_, i) =>
    entries.filter(([k]) => hash(k) === i),
  );
  ...
  return (
    <div className="bg-[#080b07] border border-[#2a3325] rounded-[12px] p-5 my-[14px]">
      <div className="flex gap-5 flex-wrap">
        {/* Buckets */}
        <div className="flex-1" style={{ minWidth: 200 }}>
          <div className="text-[#5a6e50] text-[12px] mb-2">
            해시 버킷 (크기 {bucketSize})
          </div>
          {buckets.map((bucket, i) => (
            <div key={`bucket-${i}`} className="flex items-center gap-2 mb-1">
              <div className="w-[28px] h-[34px] bg-[#1a1f16] border border-[#2a3325] rounded-[4px] flex items-center justify-center text-[#5a6e50] text-[11px] font-mono">
                {i}
              </div>
              {bucket.map(([k, v]) => (
                <div
                  key={k}
                  className="px-[10px] py-1 bg-[#7eff6a22] border border-[#7eff6a55] rounded-[6px] text-[12px] text-[#7eff6a] font-mono"
                >
                  {k}: {v}
                </div>
              ))}
              {bucket.length === 0 && (
                <div className="text-[#5a6e50] text-[12px]">—</div>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="flex-1" style={{ minWidth: 180 }}>
          ...
          <div className="flex gap-[6px] mb-3 flex-wrap">
            ...
            <button
              type="button"
              onClick={add}
              className="px-3 py-[6px] bg-[#ff8c42] border-none text-black rounded-[6px] cursor-pointer font-bold text-[12px]"
            >
              추가
            </button>
          </div>
          ...
          <div className="flex gap-[6px]">
            ...
            <button
              type="button"
              onClick={search}
              className="px-3 py-[6px] bg-[#42c9ff] border-none text-black rounded-[6px] cursor-pointer font-bold text-[12px]"
            >
              탐색
            </button>
          </div>
  ...
  return (
    <div className="bg-[#080b07] border border-[#2a3325] rounded-[12px] p-5 my-[14px]">
      <div className="flex gap-[10px] items-center mb-[14px] flex-wrap">
        <span className="text-[#dde8d6] text-[13px]">
          합 = <strong className="text-[#7eff6a]">{target}</strong> 인 쌍 찾기:
        </span>
        <select
          value={target}
          onChange={e => {
        ...
        <button
          type="button"
          onClick={run}
          className="px-4 py-[6px] bg-[#7eff6a] border-none text-black rounded-[8px] cursor-pointer font-bold"
        >
          실행
        </button>
        {steps.length > 0 && (
          <>
            <button
              type="button"
              onClick={prev}
              className="px-[10px] py-[6px] bg-[#1a1f16] border border-[#2a3325] text-[#dde8d6] rounded-[6px] cursor-pointer"
            >
              ◀
            </button>
            <button
            ...
            <span className="text-[#5a6e50] text-[12px]">
              {idx + 1}/{steps.length}
            </span>
          </>
        )}
      ...
      <div className="flex gap-[5px] mb-[14px]">
        {arr.map((v, i) => {
          ...
          return (
            <div
              key={`twoptr-${v}-${i}`}
              className="flex flex-col items-center justify-center rounded-[8px] transition-all duration-[250ms] border-2"
      ...
      {step && (
        <div
          className="px-[14px] py-[10px] rounded-[8px] text-[13px] font-mono border"
          style={{
            background: step.found ? "#7eff6a15" : W34_T.card,
            borderColor: step.found ? W34_T.accent : W34_T.border,
          }}
        >
          arr[{step.l}]({arr[step.l]}) + arr[{step.r}]({arr[step.r]}) ={" "}
          {step.sum}
          {step.found ? (
...
function W34_SlidingWindowViz() {
  ...
  const maxSum = Math.max(
    ...Array.from({ length: arr.length - k + 1 }, (_, i) =>
    ...
    <div className="bg-[#080b07] border border-[#2a3325] rounded-[12px] p-5 my-[14px]">
      <div className="flex gap-[10px] items-center mb-[14px] flex-wrap">
        ...
        <input
          ...
          className="w-[100px]"
          style={{ accentColor: W34_T.accent2 }}
        />
      ...
      <div className="flex gap-[5px] mb-[14px]">
        {arr.map((v, i) => {
          ...
          return (
            <div
              key={`slide-${i}`}
              className="flex flex-col items-center justify-center rounded-[8px] transition-all duration-[250ms] gap-[2px] border-2"
              style={{
                width: 40,
                height: 48,
            ...
            >
              <div
                className="text-[#dde8d6] font-mono"
                style={{ fontWeight: inWin ? 700 : 400 }}
              >
                {v}
              </div>
      ...
      <div className="flex gap-2 items-center mb-3">
        <button
          type="button"
          onClick={() => setWinIdx(Math.max(0, winIdx - 1))}
          className="px-3 py-[6px] bg-[#1a1f16] border border-[#2a3325] text-[#dde8d6] rounded-[6px] cursor-pointer"
        >
          ◀
        </button>
        <button
          type="button"
          onClick={() => setWinIdx(Math.min(maxStart, winIdx + 1))}
          className="px-3 py-[6px] bg-[#1a1f16] border border-[#2a3325] text-[#dde8d6] rounded-[6px] cursor-pointer"
        >
          ▶
        </button>
      ...
      <div className="flex gap-4 text-[13px] font-mono">
        <span>
          현재 합: <strong className="text-[#ff8c42]">{curSum}</strong>
        </span>
        <span>
          최대 합: <strong className="text-[#7eff6a]">{maxSum}</strong>{" "}
          {curSum === maxSum && "← ✅ 최대!"}
        </span>
      </div>
    </div>
