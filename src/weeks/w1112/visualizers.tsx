  ...
  return (
    <div className="bg-[#010610] border border-[#102040] rounded-[12px] p-[20px] my-[14px]">
      <div className="flex gap-[12px] mb-[16px] flex-wrap">
        ...
        <button type="button" onClick={doUnion} className="px-[14px] py-[6px] bg-[#00e5ff] border-none text-black rounded-[8px] cursor-pointer font-bold text-[13px]">union</button>
        <button type="button" onClick={doFind} className="px-[14px] py-[6px] bg-[#142035] border border-[#00e5ff] text-[#00e5ff] rounded-[8px] cursor-pointer text-[13px]">find</button>
        <button type="button" onClick={reset} className="px-[12px] py-[6px] bg-[#0a1628] border border-[#102040] text-[#3a6080] rounded-[8px] cursor-pointer text-[12px]">🔄 리셋</button>
      </div>

      {/* Groups visualization */}
      <div className="flex gap-[10px] flex-wrap mb-[14px]">
        {groups.map(g => (
          <div key={g.root} className="px-[14px] py-[8px] bg-[#00e5ff11] border-2 border-[#00e5ff44] rounded-[10px]">
            <div className="text-[#3a6080] text-[10px] mb-[4px]">집합 (루트:{g.root})</div>
    ...
    <div className="bg-[#010610] border border-[#102040] rounded-[12px] p-[20px] my-[14px]">
      <div className="flex gap-[16px] flex-wrap">
        <svg viewBox="0 0 370 330" className="w-full max-w-[370px] flex-none">
          {edges.map(([a,b],i) => (
            <line key={`e-${a}-${b}-${i}`} x1={nodePos[a].x} y1={nodePos[a].y} x2={nodePos[b].x} y2={nodePos[b].y}
        ...
        <div className="flex-1 min-w-[160px]">
          <div className="mb-[10px]">
            <div className="text-[#3a6080] text-[11px] mb-[4px]">큐 (진입차수 0)</div>
            <div className="flex gap-[4px] flex-wrap">
              {step.queue.length===0
                ? <span className="text-[#3a6080] text-[12px]">비어있음</span>
                : step.queue.map(n => (
                  <div key={n} className="w-[32px] h-[32px] flex items-center justify-center bg-[#ff8c0033] border-2 border-[#ff8c00] rounded-[6px] text-[#e8f4fd] font-bold font-mono text-[13px]">{n}</div>
                ))}
            </div>
          </div>
          <div className="mb-[10px]">
            ...
            <div className="flex gap-[3px] flex-wrap">
              {step.visited.map((n,i) => (
                <span key={`vis-${n}-${i}`} className="text-[#00ff88] font-mono text-[12px]">
          ...
          <div className="mt-[8px] text-[11px] flex gap-[10px]">
            <span><span className="text-[#00e5ff]">●</span> 현재</span>
            <span><span className="text-[#00ff88]">●</span> 완료</span>
            <span><span className="text-[#ff8c00]">●</span> 큐</span>
            <span className="text-[#ff8c00] font-mono text-[10px]">↗ = 진입차수</span>
          </div>
        </div>
      </div>
      <div className="flex gap-[8px] mt-[12px] flex-wrap">
        <button type="button" onClick={build} className="px-[16px] py-[7px] bg-[#00e5ff] border-none text-black rounded-[8px] cursor-pointer font-bold text-[13px]">📐 시작</button>
        <button type="button" onClick={()=>setIdx(p=>Math.max(0,p-1))} disabled={idx<=0}
          className="px-[10px] py-[7px] bg-[#0a1628] border border-[#102040] text-[#e8f4fd] rounded-[8px] cursor-pointer">◀</button>
        <button type="button" onClick={()=>setIdx(p=>Math.min(steps.length-1,p+1))} disabled={idx>=steps.length-1}
          className="px-[10px] py-[7px] bg-[#0a1628] border border-[#102040] text-[#e8f4fd] rounded-[8px] cursor-pointer">▶</button>
        {steps.length>0 && <span className="text-[#3a6080] text-[12px] self-center">{idx+1}/{steps.length}</span>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="bg-[#010610] border border-[#102040] rounded-[12px] p-[20px] my-[14px]">
      ...
      <div className="overflow-x-auto mb-[14px]">
        <svg viewBox={`0 0 ${svgW} ${(maxDepth+1)*levelH}`} className="w-full block" style={{ maxWidth: svgW }}>
      ...
      <div className="flex gap-[8px] items-center flex-wrap">
        <span className="text-[#3a6080] text-[12px]">구간 쿼리 [{queryL},{queryR}]:</span>
        ...
        <button type="button" onClick={()=>query(queryL,queryR)}
    ...
    <div className="bg-[#010610] border border-[#102040] rounded-[12px] p-[20px] my-[14px]">
      <div className="mb-[16px]">
        <div className="flex justify-between items-center mb-[8px]">
          <span className="text-[#ffd700] font-bold">전체 진도: {done}/{allTopics.length}</span>
        ...
        <div className="bg-[#0a1628] rounded-[20px] h-[8px] overflow-hidden">
          <div
            ...
            style={{ width:`${pct}%`, background:`linear-gradient(90deg,${W1112_T.w11},${W1112_T.w12})` }}
          />
        </div>
      </div>
        ...
        {allTopics.map(({week,topic}) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <div className="text-[#3a6080] text-[10px] font-mono">Week {week}</div>
                <div
                  className="text-[12px]"
                  style={{ color: isDone ? W1112_T.text : W1112_T.muted, fontWeight: isDone ? 600 : 400 }}
                >
                  {topic}
                </div>
              </div>
