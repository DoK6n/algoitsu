...
function W1112_Callout({ color = W1112_T.w11, icon = "💡", title, children }) {
  return (
    ...
    >
      ...
      <div className="text-[#e8f4fd] leading-[1.9] text-[13.5px]">
        {children}
      </div>
    </div>
  );
...
function W1112_CodeBlock({ code }) {
  ...
  const renderLine = line => {
    ...
    const tokens = src
      ...
      .map((t, i) => {
        ...
        if (kw.has(t))
          return (
            <span key={k} className="text-[#c084fc] font-bold">
              {t}
            </span>
          );
        if (bi.has(t))
          return (
            <span key={k} className="text-[#67e8f9]">
              {t}
            </span>
          );
        if (/^\d+$/.test(t))
          return (
            <span key={k} className="text-[#fbbf24]">
              {t}
            </span>
          );
        if (/^["']/.test(t))
          return (
            <span key={k} className="text-[#86efac]">
              {t}
            </span>
          );
    ...
    return [
      ...tokens,
      cmt && (
        <span key="c" className="text-[#1e3a5f] italic">
          {cmt}
        </span>
      ),
    ];
  ...
  return (
    <div className="bg-[#010610] border border-[#102040] rounded-[12px] overflow-hidden my-[12px]">
      ...
      <pre className="p-[16px] m-0 overflow-x-auto font-mono text-[13px] leading-[1.75] text-[#e8f4fd]">
        {code.split("\n").map((line, i) => {
          ...
          return (
            <div
              key={`${lineNum}-${line.trimStart().slice(0, 12)}`}
              className="flex"
            >
              <span className="text-[#3a6080] select-none min-w-[30px] text-[11px] pt-[2px]">
                {lineNum}
              </span>
...
export const W1112_sections = {
  intro: () => (
    <div>
      <W1112_SectionTitle color={W1112_T.text}>
        🗺️ 5단계 Week 11 + 6단계 Week 12 — 마지막!
      </W1112_SectionTitle>
        ...
        {[
          {
            week: "Week 11",
            title: "기타 알고리즘",
            items: ["유니온 파인드", "위상 정렬", "세그먼트 트리"],
            color: W1112_T.w11,
          },
          {
            week: "Week 12",
            title: "실전 대비",
            items: ["기업별 전략", "시간 최적화", "디버깅 패턴"],
            color: W1112_T.w12,
          },
          ...
          >
            ...
            <div className="text-[#e8f4fd] font-bold text-[15px] mb-[12px]">
              {title}
            </div>
            {items.map(i => (
              <div
                key={i}
                className="text-[#3a6080] text-[13px] py-[4px] border-b border-[#102040]"
              >
                • {i}
              </div>
            ))}
      ...
      <W1112_Callout
        ...
        title="12주 완주를 축하합니다!"
      >
        여기까지 왔다면 <strong>유니콘·대기업 코딩테스트</strong>를 충분히
        도전할 수 있는 실력입니다.
        <br />
        마지막 두 주는 나머지 핵심 알고리즘을 마무리하고, 실전 감각을
        극대화합니다.
      </W1112_Callout>
      <div className="grid gap-[10px] grid-cols-[repeat(auto-fill,minmax(170px,1fr))]">
        {[
          ...
          >
            <div className="text-[#e8f4fd] font-bold text-[13px]">
              {item.name}
            </div>
            <div className="text-[#3a6080] text-[11px] my-[4px]">
              {item.use}
            </div>
  ...
  uf: () => (
    <div>
      <W1112_SectionTitle
        sub="서로소 집합 — 합치기(Union)와 찾기(Find)"
        color={W1112_T.w11}
      >
        🔗 유니온 파인드 (Union-Find)
      </W1112_SectionTitle>
  ...
  kruskal: () => (
    <div>
      ...
      <div className="flex gap-[8px] mb-[16px] flex-wrap">
        ...
        <W1112_Badge color={W1112_T.green}>
          간선 정렬 + 유니온파인드
        </W1112_Badge>
      </div>
      <W1112_Callout color={W1112_T.w11} icon="🧠" title="MST 핵심 아이디어">
        <strong>모든 노드를 최소 비용의 간선으로 연결</strong>하는 트리
        <br />
        크루스칼: 간선을 <strong>가중치 오름차순</strong>으로 정렬 후, 사이클을
        만들지 않는 간선만 추가
      </W1112_Callout>
      <W1112_CodeBlock
        code={`# ─── 크루스칼 MST ───
...
def solution(n, costs):
    ...
    return answer`}
      ...
      <div className="grid grid-cols-2 gap-[12px]">
        <W1112_Callout color={W1112_T.w11} icon="🌲" title="크루스칼 vs 프림">
          <strong>크루스칼</strong>: 간선 중심, 희소 그래프 유리, O(E log E)
          <br />
          <strong>프림</strong>: 노드 중심, 밀집 그래프 유리, O(V²) 또는 O(E log
          V)
        </W1112_Callout>
  ...
  topo: () => (
    <div>
      <W1112_SectionTitle
        sub="의존 관계가 있는 순서 결정 — BFS(Kahn) 방식"
        color={W1112_T.w11}
      >
      ...
      <div className="flex gap-[8px] mb-[16px] flex-wrap">
        ...
        <W1112_Badge color={W1112_T.orange}>
          방향 비순환 그래프(DAG)에서만
        </W1112_Badge>
      </div>
      <W1112_TopoViz />
...
def topological_sort_dfs(n, graph):
    ...
    return stack[::-1]  # 역순이 위상 정렬`}
      />
      <W1112_CodeBlock
        code={`# ─── 실전: BOJ 1005 ACM Craft (위상 정렬 + DP) ───
...
def acm_craft():
  ...
  segtree: () => (
    <div>
      <W1112_SectionTitle
        sub="구간 합/최솟값을 O(log N)에 — 배열 업데이트도 O(log N)"
        color={W1112_T.w11}
      >
      ...
      <W1112_Callout
        color={W1112_T.w11}
        icon="🔑"
        title="누적합 vs 세그먼트 트리 선택"
      >
        <strong>누적합(prefix sum)</strong>: 쿼리 O(1), 업데이트 O(N) — 수정
        없을 때<br />
        <strong>세그먼트 트리</strong>: 쿼리 O(log N), 업데이트 O(log N) — 수정
        빈번할 때<br />
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-[12px] mb-[16px]">
        {[
          ...
          <div
            key={title}
            className="bg-[#0a1628] border border-[#102040] rounded-[10px] p-[16px]"
          >
            <div className="font-bold mb-[10px] text-[14px]" style={{ color }}>
              {title}
            </div>
            {items.map(item => (
              <div
                key={item}
                className="text-[#3a6080] text-[12px] py-[4px] border-b border-[#102040] flex gap-[8px]"
              >
...
big = 10**18 * 10**18  # 아무 문제 없음`}
        ...
        {[
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-[10px]">
              <div>
                <div className="text-[#3a6080] text-[11px] mb-[6px]">
                  출제 특징
                </div>
                {co.features.map(f => (
                  <div key={f} className="text-[#e8f4fd] text-[12px] py-[2px]">
                    • {f}
                  </div>
                ))}
              </div>
              <div>
                ...
                {co.problems.map(p => (
                  <div key={p} className="text-[#3a6080] text-[12px] py-[2px]">
                    📌 {p}
                  </div>
                ))}
                <div
  ...
  checklist: () => (
    <div>
      ...
      {[
        ...
        <div
          key={section.title}
          className="mb-[16px] bg-[#0a1628] border border-[#102040] rounded-[12px] p-[16px]"
        >
          <div
            className="font-bold mb-[10px] text-[14px]"
  ...
  complete: () => (
    <div className="text-center py-[20px]">
      ...
      <h1 className="text-[#ffd700] text-[32px] font-black font-mono mb-[8px] [text-shadow:0_0_40px_#ffd70088]">
        12주 커리큘럼 완주!
      </h1>
      ...
      <div className="grid gap-[12px] max-w-[800px] mx-auto mb-[32px] text-left grid-cols-[repeat(auto-fill,minmax(220px,1fr))]">
        {[
          ...
          >
            ...
            <div className="text-[#e8f4fd] font-bold text-[13px] mt-[2px]">
              {item.title}
            </div>
          </div>
        ))}
      ...
      <div className="max-w-[600px] mx-auto">
        <W1112_Callout color={W1112_T.w12} icon="🚀" title="다음 단계">
          <strong>1단계</strong>: 프로그래머스 레벨 3 문제 20개 풀기
          <br />
          <strong>2단계</strong>: 목표 기업 기출 5년치 전부 풀기
          <br />
          <strong>3단계</strong>: 주 2회 모의 코딩테스트 (시간 제한 엄수)
          <br />
        ...
        <div className="mt-[24px] p-[20px] bg-[#0a1628] rounded-[12px] flex flex-col gap-[8px] border border-[#ffd70044]">
          ...
          {[
            ...
            <div
              key={r.name}
              className="flex gap-[10px] py-[8px] border-b border-[#102040]"
            >
              <span className="text-[#ffd700] text-[14px]">→</span>
              <div>
                <span className="text-[#e8f4fd] font-semibold text-[13px]">
                  {r.name}
                </span>
                <span className="text-[#3a6080] text-[12px] ml-[8px]">
                  {r.desc}
                </span>
              </div>
            </div>
  ...
  problems: () => (
    <div>
      ...
      <div className="mb-[24px]">
        <div className="text-[#00e5ff] font-bold mb-[12px] text-[15px]">
          유니온 파인드 / MST
        </div>
        {[
          {
          ...
          <div
            key={p.id}
            className="flex justify-between items-center px-[16px] py-[10px] bg-[#0a1628] border border-[#102040] rounded-[10px] mb-[6px] border-l-[3px] border-l-[#00e5ff]"
          >
            <div>
              <span className="text-[#3a6080] text-[12px] font-mono">
                BOJ {p.id}
              </span>
              <span className="text-[#e8f4fd] font-bold ml-[10px]">
                {p.title}
              </span>
              <span className="text-[#3a6080] text-[12px] ml-[10px]">
                {p.tip}
              </span>
            </div>
            <span>{p.level}</span>
        ...
        {[{ title: "섬 연결하기", tip: "크루스칼 MST" }].map(p => (
          <div
            key={p.title}
            className="flex justify-between items-center px-[16px] py-[10px] bg-[#0a1628] border border-[#102040] rounded-[10px] mb-[6px] border-l-[3px] border-l-[#00e5ff88]"
          >
            <div>
      ...
      <div className="mb-[24px]">
        <div className="text-[#ff8c00] font-bold mb-[12px] text-[15px]">
          위상 정렬
        </div>
        {[
          {
          ...
          <div
            key={p.id}
            className="flex justify-between items-center px-[16px] py-[10px] bg-[#0a1628] border border-[#102040] rounded-[10px] mb-[6px] border-l-[3px] border-l-[#ff8c00]"
          >
            <div>
              <span className="text-[#3a6080] text-[12px] font-mono">
                BOJ {p.id}
        ...
        {[{ title: "순위", tip: "플로이드-워셜 or 위상 정렬" }].map(p => (
          <div
            key={p.title}
            className="flex justify-between items-center px-[16px] py-[10px] bg-[#0a1628] border border-[#102040] rounded-[10px] mb-[6px] border-l-[3px] border-l-[#ff8c0088]"
          >
            <div>
              <span className="text-[#3a6080] text-[12px]">프로그래머스</span>
              <span className="text-[#e8f4fd] font-bold ml-[10px]">
      ...
      <W1112_Callout color={W1112_T.w12} icon="🏁" title="마지막 한 마디">
        알고리즘은 <strong>양보다 질</strong>입니다.
        <br />한 문제를 풀 때 <strong>"왜 이 알고리즘인가?"</strong>를 설명할 수
        있어야 합니다.
        <br />
        모르는 문제는 30분 고민 후 풀이 보고,{" "}
        <strong>반드시 직접 다시 구현</strong>하세요.
        <br />
