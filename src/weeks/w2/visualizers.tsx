    ...
    <div className="bg-[#0d1117] border border-[#1e2d45] rounded-[12px] p-5 my-4">
          ...
          <div key={ids[i]} className="flex-1 flex flex-col items-center gap-1">
            ...
            <div
              style={{
                ...
                boxShadow: step.swapping.includes(i)
                  ? `0 0 12px ${W2_COLORS.danger}`
                  : step.comparing.includes(i)
                    ? `0 0 8px ${W2_COLORS.warn}`
                    : "none",
              }}
      ...
      <div className="flex gap-3 items-center flex-wrap">
        <button
          ...
          className="px-[14px] py-[6px] bg-[#111827] border border-[#1e2d45] text-[#e2e8f0] rounded-[6px] cursor-pointer"
        >
          ◀ 이전
        </button>
        <button
          ...
          style={{
            background: isPlaying ? W2_COLORS.danger : W2_COLORS.accent,
          }}
        >
          {isPlaying ? "⏸ 멈춤" : "▶ 재생"}
        </button>
        ...
        <button
          type="button"
          onClick={reset}
          className="px-[14px] py-[6px] bg-[#111827] border border-[#1e2d45] text-[#64748b] rounded-[6px] cursor-pointer"
        >
          🔄 리셋
        </button>
      ...
      <div className="flex gap-4 mt-3 text-[12px]">
        <span>
          <span className="text-[#f59e0b]">●</span> 비교 중
        </span>
        <span>
          <span className="text-[#ef4444]">●</span> 교환 중
        </span>
        <span>
          <span className="text-[#10b981]">●</span> 정렬 완료
        </span>
        <span>
          <span className="text-[#00d4ff]">●</span> 미정렬
        </span>
      </div>
    </div>
  );
    ...
    <div className="bg-[#0d1117] border border-[#1e2d45] rounded-[12px] p-5 my-4">
      <div className="flex gap-3 mb-5 items-center flex-wrap">
        ...
        <button
          type="button"
          onClick={runSearch}
          className="px-[18px] py-[6px] bg-[#00d4ff] border-none text-black rounded-[6px] cursor-pointer font-bold"
        >
          🔍 탐색 시작
        </button>
        {steps.length > 0 && (
          <>
            <button
              type="button"
              onClick={() => setCurrentStep(Math.max(0, currentStep - 1))}
              className="px-3 py-[6px] bg-[#111827] border border-[#1e2d45] text-[#e2e8f0] rounded-[6px] cursor-pointer"
            >
              ◀
            </button>
            <button
      ...
      <div className="flex gap-1 flex-wrap">
        {arr.map((val, i) => {
          ...
          if (step) {
            if (i === step.mid) {
              bg = step.found ? W2_COLORS.accent3 : W2_COLORS.warn;
              color = "#000";
            } else if (i === step.left) {
              border = W2_COLORS.accent;
            } else if (i === step.right) {
              border = W2_COLORS.accent2;
            } else if (i < step.left || i > step.right) {
              color = W2_COLORS.muted;
              bg = "#0a0e1a";
            }
          }
          return (
            <div
              key={val}
              className="w-[50px] h-[50px] flex flex-col items-center justify-center rounded-[8px] text-[14px] font-bold transition-all duration-300"
              style={{ background: bg, border: `2px solid ${border}`, color }}
            >
              <div>{val}</div>
              <div
                className="text-[9px]"
                style={{ color: i === step?.mid ? "#000" : W2_COLORS.muted }}
              >
                [{i}]
              </div>
            </div>
      ...
      {step && (
        <div
          className="mt-4 px-4 py-3 bg-[#111827] rounded-[8px]"
          style={{
            borderLeft: `3px solid ${step.found ? W2_COLORS.accent3 : W2_COLORS.warn}`,
          }}
        >
          <div className="text-[#e2e8f0] text-[13px]">
            <span className="text-[#00d4ff]">left={step.left}</span>,{" "}
            <span className="text-[#7c3aed]">right={step.right}</span>,{" "}
            <span className="text-[#f59e0b]">mid={step.mid}</span> → arr[
            {step.mid}]={arr[step.mid]}
            {step.found ? (
              <span className="text-[#10b981] ml-2">✅ 찾았습니다!</span>
            ) : step.tooSmall ? (
              <span className="text-[#64748b] ml-2">
                → 목표값이 더 크므로 left = mid+1
              </span>
            ) : (
              <span className="text-[#64748b] ml-2">
                → 목표값이 더 작으므로 right = mid-1
              </span>
            )}
          </div>
        </div>
...
function W2_ParametricViz() {
  ...
  const calc = height =>
    trees.reduce((sum, t) => sum + Math.max(0, t.h - height), 0);

  return (
    <div className="bg-[#0d1117] border border-[#1e2d45] rounded-[12px] p-5 my-4">
      <p className="text-[#e2e8f0] text-[13px] mt-0">
        🌲 나무 절단기 높이를 정해 {needed}m 이상 얻기 (백준 2805)
      </p>
        ...
        {trees.map(({ id, h }) => (
          <div key={id} className="flex-1 flex flex-col items-center">
            <div
              ...
              style={{
                height: `${(Math.min(h, mid) / 8) * 80}px`,
                background: W2_COLORS.accent3,
              }}
            />
            {h > mid && (
              <div
                ...
                style={{
                  height: `${((h - mid) / 8) * 80}px`,
                  background: W2_COLORS.warn,
                }}
              />
            )}
          </div>
      ...
      <div className="flex gap-[6px] mb-2 text-[12px]">
        <span>
          <span className="text-[#10b981]">●</span> 남은 나무
        </span>
        <span>
          <span className="text-[#f59e0b]">●</span> 잘린 나무 (획득)
        </span>
      </div>
      <div className="flex items-center gap-3">
        ...
        <input
          ...
          className="flex-1"
          style={{ accentColor: W2_COLORS.accent }}
        />
      ...
      <div className="mt-3 px-[14px] py-[10px] bg-[#111827] rounded-[8px] text-[13px]">
        ...
        <span
          className="font-bold"
          style={{
            color: calc(mid) >= needed ? W2_COLORS.accent3 : W2_COLORS.danger,
          }}
        >
          {calc(mid)}m
        </span>
        <span className="text-[#64748b] ml-2">
          (필요: {needed}m → {calc(mid) >= needed ? "✅ 가능!" : "❌ 부족"})
        </span>
      </div>
    </div>
  );
...
function W2_ComplexityBadge({ time, space }) {
  return (
    <span className="inline-flex gap-2 text-[12px]">
      <span className="bg-[#1a2744] border border-[#00d4ff] text-[#00d4ff] px-2 py-[2px] rounded-[20px]">
        시간 {time}
      </span>
      <span className="bg-[#1a1744] border border-[#7c3aed] text-[#7c3aed] px-2 py-[2px] rounded-[20px]">
        공간 {space}
      </span>
    </span>
  );
