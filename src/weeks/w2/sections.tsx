...
export const W2_sectionData = {
  intro: () => (
    <div>
      <h2 className="text-[#00d4ff] border-b-2 border-[#1e2d45] pb-3">📋 Week 2: 정렬 & 탐색</h2>
      <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-3 mt-5">
        {[
          { name: "버블 정렬", complexity: "O(n²)", cls: "border-l-[#ef4444] text-[#ef4444]" },
          { name: "선택 정렬", complexity: "O(n²)", cls: "border-l-[#ef4444] text-[#ef4444]" },
          { name: "삽입 정렬", complexity: "O(n²)", cls: "border-l-[#f59e0b] text-[#f59e0b]" },
          { name: "병합 정렬", complexity: "O(n log n)", cls: "border-l-[#10b981] text-[#10b981]" },
          { name: "퀵 정렬", complexity: "O(n log n)*", cls: "border-l-[#10b981] text-[#10b981]" },
          { name: "힙 정렬", complexity: "O(n log n)", cls: "border-l-[#10b981] text-[#10b981]" },
          { name: "이진 탐색", complexity: "O(log n)", cls: "border-l-[#00d4ff] text-[#00d4ff]" },
          { name: "파라메트릭", complexity: "O(log n × f(n))", cls: "border-l-[#7c3aed] text-[#7c3aed]" },
        ].map(item => (
          <div key={item.name} className={`bg-[#111827] border border-[#1e2d45] rounded-[10px] px-4 py-[14px] border-l-[3px] ${item.cls.split(" ")[0]}`}>
            <div className="text-[#e2e8f0] font-bold">{item.name}</div>
            <div className={`text-[13px] mt-1 ${item.cls.split(" ")[1]}`}>{item.complexity}</div>
          </div>
        ))}
      </div>
      <div className="mt-6 p-4 bg-[#111827] rounded-[10px] border-l-[3px] border-l-[#10b981]">
        ...
        <ul className="text-[#e2e8f0] leading-[2] m-0 pl-5">
          <li>파이썬 내장 <code className="text-[#00d4ff]">sort()</code>는 Timsort (O(n log n)) - 실무에선 이걸 씁니다</li>
  ...
  bubble: () => (
    <div>
      ...
      <div className="text-[#64748b] mb-4">인접한 두 원소를 비교해 큰 값을 오른쪽으로 "버블처럼" 올립니다.</div>
...
print(bubble_sort(arr))  # [11, 12, 22, 25, 34, 64, 90]`} />
  ...
  selection: () => (
    <div>
      ...
      <div className="text-[#64748b] mb-4">매 단계에서 가장 작은 원소를 선택해 맨 앞에 배치합니다.</div>
      <W2_ComplexityBadge time="O(n²)" space="O(1)" />
...
print(selection_sort(arr))  # [11, 12, 22, 25, 64]`} />
  ...
  insertion: () => (
    <div>
      ...
      <div className="text-[#64748b] mb-4">카드를 하나씩 뽑아 적절한 위치에 삽입하는 방식입니다.</div>
...
print(insertion_sort(arr))  # [5, 6, 11, 12, 13]`} />
  ...
  merge: () => (
    <div>
      ...
      <div className="text-[#64748b] mb-4">분할 정복 전략: 반으로 나눠 각각 정렬 후 병합합니다.</div>
...
print(merge_sort(arr))  # [3, 9, 10, 27, 38, 43, 82]`} />
  ...
  quick: () => (
    <div>
      ...
      <div className="text-[#64748b] mb-4">피벗을 기준으로 좌우를 분할하는 분할 정복 정렬입니다.</div>
      ...
      <div className="p-4 bg-[#111827] rounded-[10px] my-3 font-mono text-[13px]">
        <div className="text-[#e2e8f0]">[3, 6, 8, 10, 1, 2, <span className="text-[#f59e0b] font-bold">1</span>]  피벗: 1</div>
        <div className="text-[#64748b] my-[6px]">→ 피벗보다 작은 값: [] | 피벗: [1] | 피벗보다 큰 값: [3, 6, 8, 10, 2]</div>
...
print(quick_sort(arr))  # [1, 5, 7, 8, 9, 10]`} />
  ...
  heap: () => (
    <div>
      ...
      <div className="text-[#64748b] mb-4">힙 자료구조를 이용한 정렬. 파이썬에선 heapq 모듈로 간단 구현!</div>
      <W2_ComplexityBadge time="O(n log n)" space="O(1)" />
      <div className="p-4 bg-[#111827] rounded-[10px] my-3">
        <div className="text-[#e2e8f0] text-[13px]">
          <strong className="text-[#00d4ff]">힙(Heap)이란?</strong><br />
          <span className="text-[#64748b]">최솟값(min heap) 또는 최댓값(max heap)이 항상 루트에 위치하는 완전 이진 트리</span>
        </div>
        <div className="text-center font-mono mt-3 text-[14px]">
          ...
          <div className="flex justify-center gap-[10px] mt-1">
            <span className="text-[#64748b]">9</span><span className="text-[#64748b]">5</span>
            <span className="text-[#64748b]">4</span><span className="text-[#64748b]">7</span>
          </div>
        </div>
      </div>
...
print(max_heap_sort(arr))    # [9, 6, 5, 4, 3, 2, 1, 1]`} />
      <div className="bg-[#111827] rounded-[10px] p-4 border-l-[3px] border-l-[#10b981]">
        ...
        <ul className="text-[#e2e8f0] leading-[2] mt-2">
          ...
          <li>힙 정렬 자체보다 <code className="text-[#00d4ff]">heapq</code> 사용법이 더 중요!</li>
        </ul>
      </div>
    </div>
  ),
  ...
  binary: () => (
    <div>
      ...
      <div className="text-[#64748b] mb-4">정렬된 배열에서 반씩 줄여가며 탐색. O(log n)의 마법!</div>
...
print(binary_search(arr, 6))   # -1 (없음)`} />
      <div className="bg-[#111827] rounded-[10px] p-4 border-l-[3px] border-l-[#ef4444]">
        ...
        <ul className="text-[#e2e8f0] leading-[2] mt-2">
          <li><code className="text-[#f59e0b]">while left {"<="} right</code> ← {"<"} 가 아니라 {"<="} 임에 주의!</li>
          <li>중간값: <code className="text-[#f59e0b]">{"mid = (left + right) // 2"}</code></li>
    ...
    <div>
      ...
      <div className="p-4 bg-[#111827] rounded-[10px] my-3">
        ...
        <div className="text-[#e2e8f0] leading-[1.9]">
          <span className="text-[#f59e0b]">문제:</span> "조건을 만족하는 최적값 구하기"<br />
          <span className="text-[#00d4ff]">→ 이진 탐색:</span> "이 값이 조건을 만족하는가?" (예/아니오 판단)<br />
          <span className="text-[#10b981]">→ 만족하면</span> 더 최적값을 찾고, <span className="text-[#ef4444]">불만족이면</span> 범위 조정
        </div>
      </div>
      <W2_ParametricViz />
...
print(solution(6, [7, 10]))  # 28`} />
      <div className="bg-[#111827] rounded-[10px] p-4 border-l-[3px] border-l-[#7c3aed]">
        <div className="text-[#7c3aed] font-bold">🔑 파라메트릭 서치 패턴 인식법</div>
        <ul className="text-[#e2e8f0] leading-[2] mt-2">
          <li>"최대 몇 ... 이하에서" / "최소 몇 ... 이상에서" 조건</li>
          <li>답의 범위가 연속적인 정수일 때</li>
          <li>"가능 여부"를 O(n)으로 빠르게 판단할 수 있을 때</li>
          <li>키워드: <em className="text-[#f59e0b]">최소화, 최대화, 결정</em></li>
        </ul>
      </div>
    </div>
  ),
  ...
  summary: () => (
    <div>
      ...
      <div className="bg-[#111827] rounded-[10px] p-4 mb-4">
        <div className="text-[#10b981] font-bold mb-3">⚡ 복잡도 한눈에 보기</div>
        <table className="w-full border-collapse text-[13px]">
          <thead>
            <tr className="border-b border-[#1e2d45]">
              {["알고리즘", "평균", "최악", "공간", "안정?"].map(h => (
                <th key={h} className="text-[#64748b] px-3 py-2 text-left">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {[
              ...
              <tr key={i} className={`border-b border-[#1e2d45] ${i % 2 !== 0 ? "bg-[#0d1117]" : ""}`}>
                <td className="text-[#e2e8f0] px-3 py-2 font-bold">{name}</td>
                <td className={`px-3 py-2 ${avg.includes("n²") ? "text-[#ef4444]" : "text-[#10b981]"}`}>{avg}</td>
                <td className={`px-3 py-2 ${worst.includes("n²") ? "text-[#ef4444]" : "text-[#10b981]"}`}>{worst}</td>
      ...
      <div className="bg-[#111827] rounded-[10px] p-4 mb-4">
        ...
        {[
          ...
          <div key={i} className="flex justify-between items-center py-[10px] border-b border-[#1e2d45]">
            <div>
              <span className="text-[#e2e8f0] font-bold">{p.title}</span>
              <span className="text-[#64748b] text-[12px] ml-[10px]">{p.desc}</span>
            </div>
