...
function tokenize(text: string): React.ReactNode[] {
  return text.split(/(\b\w+\b|[^\w\s]|\s+)/g).filter(Boolean).map((t, i) => {
    ...
    if (KEYWORDS.has(t)) return <span key={`kw-${i}-${t}`} className="text-[#c084fc] font-bold">{t}</span>
    // eslint-disable-next-line react/no-array-index-key
    if (BUILTINS.has(t)) return <span key={`bi-${i}-${t}`} className="text-[#67e8f9]">{t}</span>
    // eslint-disable-next-line react/no-array-index-key
    if (/^\d+$/.test(t)) return <span key={`num-${i}-${t}`} className="text-[#fbbf24]">{t}</span>
    // eslint-disable-next-line react/no-array-index-key
    if (/^["']/.test(t)) return <span key={`str-${i}-${t}`} className="text-[#86efac]">{t}</span>
...
export function CodeBlock({ code, className }: CodeBlockProps) {
  ...
  const { wrapper, header, lang, copyBtn, pre, line, lineNum } = codeBlock({ copied })
